...
def diagrammatxt(now,filename,a,b,c): 
  ...
  for i in a:
    ...
    z.append(int (float(i[2])))
  plt.plot(x,y)
  plt.plot(x,z,label ="mean")
  plt.title('DDQN') # subplot 211 title
  plt.ylabel('Reward')
  plt.xlabel('Episodes')
  plt.show()
  plt.savefig("DDQN_{}.png".format(now))

def run(training_mode, pretrained, double_dqn, num_episodes=1000, exploration_max=1):
    
    now = datetime.now().strftime("%d-%m-%d_%H:%M:%S")
    print("now = ",now)
    filename = f"DDQN_{now}.txt"
    time_flag = f"2D_time_flage_{now}.txt"
    ...
    for ep_num in tqdm(range(num_episodes)):
        ...
        if ep_num != 0 and ep_num % 10 == 0:
            #
            diagrammatxt(now,filename,ep_num + 1,total_rewards[-1], np.mean(total_rewards))
            with open(time_flag,'a+') as f:
              f.write(str(ep_num + 1)+','+str(info["flag_get"])+','+str(info["time"])+'\n')
            #
            print("Episode {} score = {}, average score = {}".format(ep_num + 1, total_rewards[-1], np.mean(total_rewards)))
        # if info["flag_get"]:
        # # if random.randint(0, 10)%2 == 0: dokimastiko
        #     print("Finished, time={}, score = {} ".format(info["time"], total_rewards[-1]))    #
        #     ep_num = num_episodes                                                               #######
        #     break     
                                                                         ###########
        num_episodes += 1  
